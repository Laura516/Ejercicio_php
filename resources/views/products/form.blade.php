...
<form action="{{ route('product.save')}}" method="post">
    ...
    <!--csrf genera un campo oculto con un token-->
    <div class="row mb-3">
        <label for="name" class="col-sm-2 col-form-label">Name</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" name="name" value="{{ @old('name') ? @old('name') : $product->name }}">
            <!--Si old name tiene algo ? va a imprimirse a el mismo, pero si no tiene ningun valor almacenado el pasa a product-->
            <!--operador ternario,primero condicion + verdadero + falso-->
        </div>
    </div>
    @error('name')
